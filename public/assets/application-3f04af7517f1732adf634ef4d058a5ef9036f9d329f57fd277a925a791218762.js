...
(function() {
  ...
  chatReady = function() {
    var chatContainer, input, sendChat, socket, wrapper, wrapperChat;
    ...
    socket.onmessage = function(event) {
      ...
      if (event.data.length) {
        ...
        if (jsonData.enable_notification) {
          $('#chatAudio')[0].play();
          return $("#chat-output li").last().effect("shake", "slow");
        }
      }
    };
    ...
    return $("form.chat").on("submit", function(event) {
      event.preventDefault();
      return sendChat();
    });
  };
